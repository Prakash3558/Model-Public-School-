...
export const StudentPortal: React.FC = () => {
  ...
  // Compute attendance stats (excluding holidays from denominator)
  const totalRecordedDays = attendanceRecords.length;
  const holidayDays = attendanceRecords.filter(a => a.status === 'Holiday').length;
  const workingDays = Math.max(1, totalRecordedDays - holidayDays);
  ...
  const attendancePercentage = Number(((totalAttended / workingDays) * 100).toFixed(1));
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm text-center space-y-1.5">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Effective Attendance</span>
                <div className="text-3xl font-black text-emerald-500 font-heading">{attendancePercentage}%</div>
                <p className="text-[11px] text-slate-500 font-medium">Calculated over {workingDays} working days</p>
              </div>

              <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm text-center space-y-1.5">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Working Days Present</span>
                <div className="text-3xl font-black text-blue-500 font-heading">{totalAttended} / {workingDays} Days</div>
                <p className="text-[11px] text-slate-500 font-medium">{presentDays} Present, {lateDays} Late, {absentDays} Absent</p>
              </div>

              <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm text-center space-y-1.5 ring-2 ring-purple-500/20">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-purple-600 dark:text-purple-400">School Holidays</span>
              ...
              <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm text-center space-y-1.5">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">CBSE Eligibility Status</span>
                <div className={`text-xl font-black font-heading ${attendancePercentage >= 75 ? 'text-emerald-500' : 'text-rose-500'}`}>
                  {attendancePercentage >= 75 ? 'ELIGIBLE (75%+)' : 'LOW ATTENDANCE ALERT'}
                </div>
                <p className="text-[11px] text-slate-500 font-medium">Verified by Class Teacher</p>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div>
                  <h3 className="text-base font-bold font-heading text-slate-900 dark:text-white flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-indigo-500" /> Daily Student Attendance History & Holiday Log
                  </h3>
                  <p className="text-xs text-slate-500">Official record for Class {student.class}-{student.section}</p>
                </div>
              </div>
                ...
                <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
                  <thead className="bg-slate-100 dark:bg-slate-800 font-bold text-slate-900 dark:text-white uppercase text-[10px] tracking-wider">
                    <tr>
                      ...
                      <th className="p-3 rounded-r-xl">Teacher Remarks</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                    {attendanceRecords.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="p-8 text-center text-slate-400">No attendance logs found.</td>
                      </tr>
                    ) : (
