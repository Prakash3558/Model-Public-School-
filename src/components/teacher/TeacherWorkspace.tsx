...
export const TeacherWorkspace: React.FC = () => {
  ...
  const TOOLS_LIST: { id: TabType; title: string; subtitle: string; icon: any; color: string }[] = [
    { id: 'homework', title: 'Homework', subtitle: `${homeworkList.length} Tasks`, icon: BookOpen, color: 'text-amber-500' },
    { id: 'attendance', title: 'Attendance', subtitle: 'Daily Record', icon: Calendar, color: 'text-emerald-500' },
  ...
  return (
    <div className="min-h-screen bg-stone-100 dark:bg-slate-950 text-slate-800 dark:text-slate-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Top Header Bar */}
        <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-xl border border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
            {TOOLS_LIST.map(tool => {
              ...
              return (
                ...
                >
                  <IconComp className={`w-5 h-5 ${isActive ? 'text-slate-950' : tool.color}`} />
                  <span className="font-extrabold leading-tight">{tool.title}</span>
                  <span className={`text-[10px] font-semibold ${isActive ? 'text-slate-900/80' : 'text-slate-500 dark:text-slate-400'}`}>
                    {tool.subtitle}
                  </span>
                </button>
              );
        ...
        {activeTab === 'attendance' && (() => {
          ...
          // Monthly Summary Calculations for Class
          const monthRecords = allClassAttendance.filter(a => a.date && a.date.startsWith(summaryMonth));
          const monthUniqueDates = Array.from(new Set(monthRecords.map(a => a.date)));
          const monthHolidayDates = monthUniqueDates.filter(d => monthRecords.some(r => r.date === d && r.status === 'Holiday'));
          const totalMonthWorkingDays = Math.max(0, monthUniqueDates.length - monthHolidayDates.length);
          ...
          const studentMonthlySummaries = students.map(st => {
            ...
            const h = stRecords.filter(a => a.status === 'Holiday').length;
            const logged = stRecords.length;
            const wDays = Math.max(0, logged - h);
            const totalAtt = p + l;
            const pct = wDays > 0 ? Number(((totalAtt / wDays) * 100).toFixed(1)) : 100;
            return {
              student: st,
              logged,
              holidays: h,
